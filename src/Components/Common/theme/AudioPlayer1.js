  ...
  return (
    <div className="ap1">
      <div className="ap1-top">
        ...
        {isCurrentDurationTime && <span className="ap1-time">{formatTime(currentTime)} / {formatTime(duration)}</span>}
      </div>
      ...
      <div className="ap1-controls">
        {isNavigation && <button className="ap1-btn"> <SkipBack size={20} /> </button>}
        <button className="ap1-play" onClick={togglePlay}>
          {isPlaying ? <Pause size={24} /> : <Play size={24} />}
        </button>
        {isNavigation && <button className="ap1-btn"> <SkipForward size={20} /> </button>}
